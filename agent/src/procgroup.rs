...
use crate::protocol::{AgentMsg, LatencyPhases, RunSummary, Scenario, TelemetrySample};

/// Run the source, fanning out to child processes when `scenario.processes > 1`.
pub async fn run_source(
    ...
    if scenario.processes <= 1 {
        return engine::run_source(run_id, scenario, target_addr, tx, stop).await;
    }
    ...
    // Frames the whole group is expected to retire.
    let frame_target = scenario
        .frame
        .as_ref()
